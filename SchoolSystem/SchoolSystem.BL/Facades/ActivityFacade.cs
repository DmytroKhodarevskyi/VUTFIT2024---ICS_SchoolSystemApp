...
public class ActivityFacade : CrudFacade<ActivityEntity, ActivityListModel, ActivityDetailModel, ActivityEntityMapper>, IActivityFacade
{
        ...
        int Tag, string selectedSort, bool Descending, bool DoFilter, string Subject)
    {
        ...
        if (DoFilter)
        {
            ...
            {
                var repository = uow.GetRepository<SubjectEntity, SubjectEntityMapper>().Get();

                var subjectId = repository
                    .Where(s => s.Abbreviation == Subject)
                    .Select(s => s.Id)
                    .FirstOrDefault();

                query = query.Where(e => e.Subject != null && e.SubjectId == subjectId);
